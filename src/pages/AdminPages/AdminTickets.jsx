  ...
  const filteredData = Array.isArray(dataFetched)
    ? dataFetched?.filter(
        (item) =>
          ...
          item.ticket_link?.toLowerCase().includes(searchTerm.toLowerCase()) ||
          // item.ticket_used?.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
export default AdminOnlyRoute(AdminTickets);
